...
GuiSlideshowScreensaverOptions::GuiSlideshowScreensaverOptions(Window* window, const char* title) : GuiScreensaverOptions(window, title)
{
	...
	addWithLabel(row, "屏保切换时间", sss_image_sec);
	...
	addWithLabel(row, "全屏拉伸图片", sss_stretch);
	...
	addEditableTextComponent(row, "背景音", sss_bg_audio_file, Settings::getInstance()->getString("SlideshowScreenSaverBackgroundAudioFile"));
	...
	addWithLabel(row, "使用自定义图片", sss_custom_source);
	...
	addEditableTextComponent(row, "自定义图片路径", sss_image_dir, Settings::getInstance()->getString("SlideshowScreenSaverImageDir"));
	...
	addWithLabel(row, "图片文件夹搜索深度", sss_recurse);
	...
	addEditableTextComponent(row, "自定义图片过滤", sss_image_filter, Settings::getInstance()->getString("SlideshowScreenSaverImageFilter"));
