...
GuiGeneralScreensaverOptions::GuiGeneralScreensaverOptions(Window* window, const char* title) : GuiScreensaverOptions(window, title)
{
	...
	addWithLabel("屏保倒计时", screensaver_time);
	...
	auto screensaver_behavior = std::make_shared< OptionListComponent<std::string> >(mWindow, "屏保行为", false);
	...
	addSaveFunc([this, screensaver_behavior] {
		if (Settings::getInstance()->getString("屏保行为") != "random video" && screensaver_behavior->getSelected() == "random video") {
