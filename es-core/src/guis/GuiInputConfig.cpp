...
static const InputConfigStructure GUI_INPUT_CONFIG_LIST[inputCount] =
{
	...
	{ "HotKeyEnable",     true,  "辅助功能键",      ":/help/button_hotkey.svg" }
};
...
{
	LOG(LogInfo) << "设置" << target->getDeviceId() << "号设备" <<" (" << target->getDeviceName() << ").";
	...
	mTitle = std::make_shared<TextComponent>(mWindow, "设置", Font::get(FONT_SIZE_LARGE), 0x555555FF, ALIGN_CENTER);
	...
	mSubtitle2 = std::make_shared<TextComponent>(mWindow, "按住任意键以跳过设置", Font::get(FONT_SIZE_SMALL), 0x999999FF, ALIGN_CENTER);
	...
	if(mConfiguringRow && mHoldingInput && GUI_INPUT_CONFIG_LIST[mHeldInputId].skippable)
	{
		...
		if(mHeldTime >= HOLD_TO_SKIP_MS)
		{
			...
			if(prevSec != curSec)
			{
				...
				std::stringstream ss;
				ss << "按住" << HOLD_TO_SKIP_MS/1000 - curSec << "秒以跳过";
				text->setText(ss.str());
...
bool GuiInputConfig::assign(Input input, int inputId)
{
	...
	{
		error(mMappings.at(inputId), "键位已设置！");
		return false;
	}
