	...
	mBackground(window, ":/frame.png"), mGrid(window, Vector2i(1, 5))
{
	...
	// title
	mTitle = std::make_shared<TextComponent>(mWindow, firstRun ? "欢迎" : "输入设置",
	...
	if(numDevices > 0)
		deviceInfo << numDevices << " 个手柄" << (numDevices > 1 ? "S" : "") << "已识别";
	else
		deviceInfo << "没有识别到手柄";
	...
	// message
	mMsg1 = std::make_shared<TextComponent>(mWindow, "按住任一按键以识别设备", Font::get(FONT_SIZE_SMALL), 0x777777FF, ALIGN_CENTER);
	mGrid.setEntry(mMsg1, Vector2i(0, 2), false, true);

	const char* msg2str = firstRun ? "按F4以退出" : "按ESC以取消";
